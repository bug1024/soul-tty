...
| 能力 | 表现 |
|---|---|
| 自然语音对话 | 流式识别、流式生成、语义段 TTS；不逐字朗读，并对异常长音与提前结束自动保护和恢复 |
| 实时插话 | Serena 说话时仍能听见用户；支持打断、附和词识别、回声过滤与播放取消 |
| 稳定人格 | 名字、说话风格、行为边界、声音和视觉主题由人格配置统一描述 |
| 情绪状态 | 以愉悦、平静、好奇、压力、活力五个连续维度演化，并影响语言和声音 |
| 羁绊关系 | 关系随真实互动缓慢变化并跨启动保存，可独立查看或重置，但不做权限解锁或廉价游戏进度条 |
| 会话记忆 | 将画像、偏好和共同经历分开存储；稳定信息常驻，具体经历按需召回 |
| 主体性决策 | 回答前结合 Serena 的社交能量、交流倾向、主动性债务与未完念头，决定回答、主动带话题、自我表达、追问、转题或安静 |
...
每个语义段都有独立的完整性保护：符号和停顿会先规范化，异常长音会被限制，明显提前结束的片段不会直接播放，而会自动换用更稳妥的分段重新合成。恢复仅影响失败片段，不阻塞 LLM 主链路。
...
| Need / Agency | “我现在想怎样参与？” | 跨会话延续、每轮小幅变化 | 回答并主导、自我表达、追问、转题或主动沉默 |
...
普通社交问题不再被硬编码成 `QUESTION → ANSWER`。连续纯回答会积累
`initiative_debt`，推动 Serena 在合适时主动表达或接过下一步话语权；Reflection
产生的 `inner_thread` 保存她尚未说完的疑问与牵挂，之后可自然重新提起。它不是
长期 Memory，也不会把普通用户事实重复保存一份。
...
|---|---|
| 直接说话 | 开始一轮语音对话 |
| `0` | 普通模式循环换装；秘密模式返回进入前的普通套装 |
| `9` | 仅在秘密模式中轮换「绯夜秘仪 / 紫夜绸光 / 烟紫薄雾 / 月蚀银纱 / 月下雕像」，共用人格与语音提示词 |
| `Tab` | 在欢迎区、Serena 状态页和开发详情之间循环切换 |
| `Ctrl+C` | 安全退出 |
| `soul-tty memory` | 查看和管理长期记忆 |
| `soul-tty relationship` | 查看当前人格的亲密阶段、数值和近期关系事件 |

长期记忆和亲密关系是两套独立状态，可以分别管理：

```bash
# 长期记忆
uv run soul-tty memory list
uv run soul-tty memory clear

# 当前人格的亲密状态
uv run soul-tty relationship show --persona serena
uv run soul-tty relationship clear --persona serena
```

两种 `clear` 都需要二次确认。亲密状态清空后会恢复到该人格的初始值，同时清除互动次数与近期关系事件，不会删除长期记忆；执行前应先退出正在运行的 Soul-TTY。
